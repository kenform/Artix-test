...
import { addItem, clearItems, setItems } from '../../redux/panel/slice';
...
export default function PanelDrawer() {
	...
	const list = (anchor: Anchor) => (
		<Box sx={{ width: 480, paddingLeft: '33px' }} role='presentation'>
			...
			<PanelSelectField text='Действие' width='400px' defaultValue={''} array={actions} />
			...
			<Stack spacing={2} mt={3} mb={3} direction='row'>
				...
				<PanelButton onClick={onClicClose} modifier='outline' text='Отменить' />
			</Stack>
		</Box>
	);
			...
			{status === 'success' &&
				(['right'] as const).map((anchor) => (
					<React.Fragment key={anchor}>
						...
						<PanelButton onClick={onClickClear} modifier='purple' text='Очистить панель' />
					</React.Fragment>
				))}
		</div>
	);
